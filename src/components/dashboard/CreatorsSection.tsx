...
export function CreatorsSection() {
  ...
  const filteredCreators = creators.filter(creator => {
    const matchesSearch = creator.name.toLowerCase().includes(searchTerm.toLowerCase()) || creator.category.toLowerCase().includes(searchTerm.toLowerCase()) || creator.location.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const toggleFavorite = (creatorId: string) => {
    setCreators(prev => prev.map(creator => creator.id === creatorId ? {
      ...creator,
  ...
  return <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <Tabs value={activeTab} onValueChange={value => setActiveTab(value as "all" | "favorites")} className="w-full sm:w-auto">
          <TabsList>
        ...
        <div className="relative w-full sm:w-64">
          ...
          <Input placeholder="Buscar creador..." className="pl-10 bg-contala-green/5 border-contala-green/20" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      </div>
      
      {filteredCreators.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredCreators.map(creator => <Card key={creator.id} className="overflow-hidden hover:shadow-lg transition-all duration-300">
              <CardHeader className="pb-2 px-0">
                <div className="flex justify-between">
                  <div className="flex items-center gap-3">
                    <Avatar className="h-12 w-12 border-2 border-contala-green/10">
                      <AvatarImage src={creator.avatar} alt={creator.name} className="object-fill" />
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-3 mt-2 text-sm">
                  {creator.lastContact && <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-gray-400" />
                      <span>Último contacto: {formatDate(creator.lastContact)}</span>
                    </div>}
                  {creator.collaborations !== undefined && <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-gray-400" />
                      <span>{creator.collaborations} colaboración{creator.collaborations !== 1 ? 'es' : ''}</span>
                    </div>}
                </div>
              </CardContent>
              <CardFooter className="border-t pt-4 flex justify-between py-[10px] rounded-sm px-[7px]">
                <Button variant={creator.favorite ? "destructive" : "outline"} className={creator.favorite ? "bg-red-100 text-red-600 hover:bg-red-200 border-red-200" : ""} size="sm" onClick={() => toggleFavorite(creator.id)}>
                  {creator.favorite ? <>
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" className="text-blue-600 hover:bg-blue-50" onClick={() => toast({
              title: "Mensajería",
              description: "Función de mensajería próximamente"
                  ...
                  <Button variant="secondary" size="sm" className="bg-contala-green/10 text-contala-green hover:bg-contala-green/20" onClick={() => viewCreatorProfile(creator)}>
          ...
          <CardHeader className="flex flex-row items-center justify-center p-6">
            <div className="flex flex-col items-center text-center">
              ...
              <CardDescription>
                {activeTab === "favorites" ? "No tienes creadores favoritos. Añade algunos para verlos aquí." : "No tienes creadores en tu red todavía. Explora la sección 'Descubrir' para encontrar creadores."}
              </CardDescription>
            </div>
          </CardHeader>
        </Card>}
