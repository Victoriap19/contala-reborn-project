      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className={`transition-all duration-1000 ${isVisible ? "opacity-100" : "opacity-0 translate-y-10"}`}>
            <div className="relative">
              <div className="absolute -left-8 -top-8 w-20 h-20 bg-contala-pink/20 rounded-full animate-pulse-slow"></div>
              <div className="absolute -right-4 -bottom-4 w-16 h-16 bg-contala-navy/10 rounded-full animate-float"></div>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <Button 
                className="bg-contala-navy hover:bg-contala-navy/90 text-white h-14 px-8 text-lg rounded-md shadow-none"
          ...
          <div className={`relative h-[500px] transition-all duration-1000 ${isVisible ? "opacity-100" : "opacity-0 translate-y-10"}`}>
            ...
            {imageItems.map((item, index) => (
              <div 
                key={index}
                className={`absolute inset-0 rounded-2xl shadow-xl overflow-hidden transition-all duration-700 ${
                  activeImage === index 
        ...
        <div className="mt-24 border-t border-b border-gray-100 py-8">
          ...
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16">
            {['Acme Co.', 'GlobalTech', 'Fusión', 'Natura', 'InnovateLab'].map((name, i) => (
              <div key={i} className="text-gray-300 font-bold text-xl">
                {name}
              </div>
            ))}
          </div>
